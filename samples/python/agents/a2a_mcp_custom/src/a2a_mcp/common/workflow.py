...
class WorkflowNode:
    ...
    async def run_node(
        ...
        # Increase timeout for LLM agent processing (default is 5s which is too short)
        timeout = httpx.Timeout(300.0, connect=10.0)
        async with httpx.AsyncClient(timeout=timeout) as httpx_client:
            # Use A2AClient directly with send_message_streaming (the original working approach)
            client_instance = A2AClient(httpx_client, agent_card)

            # Create streaming message request
            # Note: taskId and contextId are NOT included in the message
            # The downstream agent will create its own task
            ...
            # Use send_message_streaming which returns an async generator
            response_stream = client_instance.send_message_streaming(request)
            async for chunk in response_stream:
                # Save artifacts if present in the response
                if isinstance(
